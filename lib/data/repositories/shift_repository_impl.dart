...
class ShiftRepositoryImpl implements ShiftRepository {
  ...
  Future<List<ShiftModel>> getShifts({
    ...
    try {
      ...
      print('✅ [ShiftRepository] getShifts SUCCESS! Raw response payload: $rawList');
      return rawList.map(ShiftModel.fromJson).toList();
    } on DioException catch (e) {
      print('❌ [ShiftRepository] getShifts FAILED! Error data: ${e.response?.data} | Exception: $e');
      throw ApiException(
  ...
  Future<ShiftModel> getShiftById(String shiftId) async {
    try {
      ...
      print('✅ [ShiftRepository] getShiftById SUCCESS! Raw response payload: $raw');
      return ShiftModel.fromJson(raw);
    } on DioException catch (e) {
      print('❌ [ShiftRepository] getShiftById FAILED! Error data: ${e.response?.data} | Exception: $e');
      throw ApiException(
        message: e.message ?? 'Lỗi kết nối máy chủ',
        statusCode: e.response?.statusCode,
      );
  ...
  Future<ShiftModel> checkIn(String shiftId) async {
    try {
      ...
      if (!serviceEnabled) {
        throw const ApiException(message: 'Dịch vụ định vị đang tắt. Vui lòng bật GPS.');
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          throw const ApiException(message: 'Bạn đã từ chối quyền truy cập vị trí. Không thể chấm công.');
        }
      }
      
      if (permission == LocationPermission.deniedForever) {
        throw const ApiException(message: 'Quyền vị trí đã bị từ chối vĩnh viễn. Vui lòng mở Cài đặt để cấp quyền.');
      }
      ...
      print('✅ [ShiftRepository] Check-in API SUCCESS! Raw response payload: $raw');

      return ShiftModel.fromJson(raw);
    } on ApiException {
      rethrow;
    } on DioException catch (e) {
      print('❌ [ShiftRepository] Check-in API FAILED! Error data: ${e.response?.data} | Exception: $e');
      throw ApiException(
